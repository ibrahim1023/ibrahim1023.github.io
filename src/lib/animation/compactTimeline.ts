...
export function buildCompactSettleTimeline(e: SettleDiffTimelineElements, layout: NarrativeLayout) {
  ...
  if (layout === "mobile" && e.artifactScene) tl.to(e.artifactScene, { y: -8, duration: .9 }, .9);
  ...
  if (layout === "mobile" && e.verified) tl.to(e.verified, { y: -8, duration: .5 }, 5.5);
  else tl.to({}, { duration: .5 }, 5.5);
  return tl;
}
