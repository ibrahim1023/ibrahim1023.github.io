...
export function buildCaseZeroNarrativeTimeline(e: CaseZeroTimelineElements, layout: NarrativeLayout): gsap.core.Timeline {
  ...
  if (e.caseFile) tl.fromTo(e.caseFile, layout === "desktop" ? { opacity: 0, visibility: "hidden", rotateY: -7, y: 24 } : { opacity: 0, visibility: "hidden", y: 42 }, { id: layout === "desktop" ? "case-file-open" : "mobile-case-file-open", opacity: 1, visibility: "visible", rotateY: 0, y: 0, duration: docket.duration * .3, ease: "power2.out" }, docket.start + docket.duration * .08);
  const evidence = caseTiming("evidence-typed");
  if (e.evidenceLayers) tl.fromTo(e.evidenceLayers, layout === "desktop" ? { opacity: .42, x: -18 } : { opacity: .42, y: 18 }, { id: layout === "desktop" ? "evidence-lineage" : "mobile-evidence-lineage", opacity: 1, x: 0, y: 0, stagger: .12, duration: evidence.duration * .52 }, evidence.start + evidence.duration * .14);
  ...
  if (e.blindScene) tl.to(e.blindScene, { opacity: 0, visibility: "hidden", duration: blind.duration * .18 }, blind.start);
  if (e.header) tl.to(e.header, { opacity: 0, visibility: "hidden", duration: blind.duration * .16 }, blind.start);
  if (e.blindClimax) tl.fromTo(e.blindClimax, { opacity: 0, scale: .92 }, { id: "blind-climax", opacity: 1, visibility: "visible", scale: 1, duration: blind.duration * .3, ease: "power2.out" }, blind.start + blind.duration * .1);
  const lock = caseTiming("lock-ready");
  if (e.blindClimax) tl.to(e.blindClimax, { opacity: 0, visibility: "hidden", duration: lock.duration * .18 }, lock.start);
  if (e.lockRecord) tl.fromTo(e.lockRecord, { opacity: 0, visibility: "hidden", y: layout === "desktop" ? 34 : 46 }, { id: "lock-record", opacity: 1, visibility: "visible", y: 0, duration: lock.duration * .3 }, lock.start + lock.duration * .1);
