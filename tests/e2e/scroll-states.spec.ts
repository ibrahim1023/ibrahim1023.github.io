...
const STATES = [
  ...
  [0.73, "[data-proof]"], [0.84, "[data-checks]"], [0.93, "[data-verified]"], [0.99, "[data-vault-transition]"],
] as const;
...
test("each settled desktop state clears the previous foreground", async ({ page }) => {
  ...
  const pairs = [
    ...
    [0.84, "[data-checks]", "[data-proof]"], [0.93, "[data-verified]", "[data-checks]"], [0.99, "[data-vault-transition]", "[data-verified]"],
  ] as const;
...
test("the evidence packet releases into the stable Vault arrival", async ({ page }) => {
  const diagnostics = capturePageDiagnostics(page);
  await page.setViewportSize({ width: 1440, height: 900 }); await page.goto("/portfolio/"); await ready(page, "desktop");
  await scrollNarrativeTo(page, "desktop", .99);
  await expectMostlyVisible(activeNarrativeLocator(page, "desktop", "[data-evidence-packet]"), { viewportRatio: .02 });
